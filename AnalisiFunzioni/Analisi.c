...
for(j = - interv; j < interv; j += ddx) {
  ...
  if((fabs(f(j+ddx)-f(j-ddx)) > eps1) && (((f(j+ddx) > 0) && (f(j-ddx) < 0)) || ((f(j+ddx) < 0) && (f(j-ddx) > 0)))){ 
   fprintf(out, "La funzione ha un punto di non definizione del dominio in %.5lf\n", j);
  j += ddx;
  contr++;
  }

//  FIXME: Correggere la logica dell'if 
  if(fabsf((j+ddx)-f(j)) > eps  ){
  fprintf(out, "La funzione ha un punto di discontinuità non risolvibile del dominio in %.5lf\n", j);
  contr++;
  }
